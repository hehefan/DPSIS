...
from models.resnet_32 import ResNet50
...
testtransform = transforms.Compose([transforms.Resize(size=cfg.resize),
                                    ...
                                    transforms.ToTensor(),
                                    transforms.Normalize(cfg.means, cfg.stds)])
trainset = cifar.CIFAR100(root=cfg.data_root, train=True, download=True, transform=traintransform)
trainloader = torch.utils.data.DataLoader(trainset, batch_size=cfg.batch_size, shuffle=True, num_workers=cfg.workers_num)
testset = cifar.CIFAR100(root=cfg.data_root, train=False, download=True, transform=testtransform)
...
#-------------------------------------------------------------Network--------------------------------------------------------------
net = ResNet50(cfg.num_features).to(cfg.device)
